<template>
  <div class="scores-page">
    <h1>Running Game Scores for {{ user.username }}</h1>
    <div class="user-info">
    ...
    <div v-if="scores.length > 0">
      <ul>
        <li v-for="(score, index) in scores" :key="index">
          {{ score.Points }} - {{ new Date(score.CreatedAt).toLocaleString() }}
        </li>
      </ul>
    </div>
    <div v-else>
      <p>No running game scores found.</p>
    </div>
    <div class="buttons">
...
export default {
  ...
  setup() {
    ...
    // Fetch running game scores securely
    const fetchScores = async () => {
      ...
      try {
        ...
        const response = await fetch(`${apiUrl}/api/runningScores`, {
          headers: {
        ...
        if (response.ok) {
          ...
          console.log("Running game scores fetched successfully:", scores.value);
        } else {
          const errorData = await response.json();
          console.error("Failed to fetch running game scores:", errorData);
        }
      } catch (error) {
        console.error("Error fetching running game scores:", error);
      }
    };
